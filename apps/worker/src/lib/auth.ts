...
export async function verifyJwt(token: string, jwksUrl: string): Promise<JwtPayload> {
  ...
  // NOTE: Web Crypto ECDSA requires namedCurve on import,
  // and its signature format is raw r||s — same as JOSE/JWT ES256.
  // No DER conversion needed (DER makes verify fail!).
